...
public:
	...
	void min(cv::UMat& dst, const ZRange& range, ReconOutput output=RECON_OUTPUT_AMPLITUDE);
	void reconMin(std::vector<cv::UMat>& dst, cv::UMat& dstMin, const ZRange& range, ReconOutput output=RECON_OUTPUT_AMPLITUDE);
